  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = isEdit
        ? await axios.put(`/api/colleges/${editcollege.id}`, form, authH())
        : await axios.post("/api/colleges", form, authH());
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription className="text-blue-200 text-xs mt-0.5">
                {isEdit ? "Update college information" : "Create a new academic college"}
              </DialogDescription>
            </div>
          </div>
        </div>
...
function DeleteDialog({ open, onClose, onConfirm, college }) {
  const canDelete = college && college.users_count === 0 && college.organizations_count === 0;
      ...
      <AlertDialogContent className="rounded-2xl border-0 shadow-2xl max-w-sm">
        <AlertDialogHeader>
          ...
          <AlertDialogTitle className="text-center text-slate-900">
            {canDelete ? "Delete this college?" : "Cannot Delete college"}
          </AlertDialogTitle>
          <AlertDialogDescription className="text-center text-slate-500 text-sm">
            {canDelete ? (
              <>
                This will permanently delete <strong className="text-slate-700">{college?.name}</strong>. This action cannot be undone.
              </>
            ) : (
              <>
                <strong className="text-slate-700">{college?.name}</strong> has{' '}
                <strong className="text-amber-600">{college?.users_count || 0} student(s)</strong> and{' '}
                <strong className="text-amber-600">{college?.organizations_count || 0} organization(s)</strong>. 
...
export default function CollegeManagement() {
  ...
  const fetchcolleges = async () => {
    try {
      setLoading(true);
      const res = await axios.get("/api/colleges", authH());
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/api/colleges/${deleteTarget.id}`, authH());
      ...
      console.error(err);
      const errorMsg = err.response?.data?.message || "Failed to delete college.";
      toast.error("Error", {
  ...
  const totalStudents = colleges.reduce((sum, d) => sum + (d.users_count || 0), 0);
  const totalOrgs = colleges.reduce((sum, d) => sum + (d.organizations_count || 0), 0);

  const filtered = colleges.filter(d => {
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-[#0f2d5e] tracking-tight">College Management</h1>
              <p className="text-slate-500 text-xs mt-0.5">Manage academic colleges and their associations</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => { seteditCollege(null); setFormOpen(true); }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <StatCard icon={Building2} label="Total colleges" value={total} sub="Active colleges" grad="from-[#0f2d5e] to-[#1a4a8a]" />
          <StatCard icon={Users} label="Total Students" value={totalStudents} sub="Across all colleges" grad="from-[#1e4db7] to-[#3b6fd4]" />
          <StatCard icon={Briefcase} label="Organizations" value={totalOrgs} sub="college organizations" grad="from-[#2563eb] to-[#5b9ef7]" />
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/60">
                  <th className="px-5 py-3 text-[11px] font-bold uppercase tracking-wider text-slate-400">college</th>
                  ...
                  <tr>
                    <td colSpan="4" className="py-24 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <span className="text-sm text-slate-400 font-medium">Loading collegesâ€¦</span>
                      </div>
                    </td>
                  </tr>
                ) : filtered.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="py-24 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-slate-500 text-sm">No colleges found</p>
                          <p className="text-xs text-slate-400 mt-0.5">Try adjusting your search or create a new college</p>
                        </div>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400 mt-0.5">college ID: {dept.id}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-40 rounded-xl border-slate-100 shadow-lg p-1">
                          <DropdownMenuItem onClick={() => { seteditCollege(dept); setFormOpen(true); }}
          ...
          <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between text-xs text-slate-400">
            <span>
              Showing <strong className="text-slate-600">{filtered.length}</strong> of <strong className="text-slate-600">{total}</strong> colleges
              {search && ` Â· "${search}"`}
            </span>
            <span>
              <strong className="text-slate-600">{totalStudents}</strong> total students Â· <strong className="text-slate-600">{totalOrgs}</strong> organizations
            </span>
          </div>
        </div>
      </div>
