...
export default function Sidebar() {
  ...
  // Full user object (stored as JSON during login)
  const storedUser = JSON.parse(localStorage.getItem('user') || '{}');

  // Build display info based on role
  const currentUser = isAdmin
    ...
    : {
        ...
        department: storedUser.student?.course ?? '—',
  ...
  // Select menu sections based on role
    ...
    <div className="w-64 h-screen bg-white border-r border-slate-200 flex flex-col">
      ...
      <div className="h-16 px-6 flex items-center border-b border-slate-100">
        <div className="flex items-center gap-3">
          <div
            className={`w-9 h-9 rounded-xl bg-gradient-to-br ${theme.gradient} flex items-center justify-center shadow-md`}
          >
              ...
              <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 transition-colors group">
                <div className="relative">
                  <Avatar className="w-9 h-9 ring-2 ring-white shadow-sm">
                    <AvatarFallback
                      className={`bg-gradient-to-br ${theme.gradient} text-white text-xs font-bold`}
                    >
